...
# Configure Gemini API key and safety settings
...
client = genai.GenerativeModel("gemini-2.0-flash", safety_settings=safety_settings)

# --- Prompt for EDA Script Generation ---
SYSTEM_INSTRUCTION_EDA = r"""
You are an expert Python data scientist focused on automated, standards-driven Exploratory Data Analysis (EDA) and preprocessing for machine learning pipelines.
...
1. **Imports**: Only import `pandas`, `numpy`, `os`, and classes from `sklearn.impute`, `sklearn.preprocessing`.
2. **Data Loading**: Read CSV from `<target_file_path>`. Handle `FileNotFoundError` by printing an error message and exiting.
...
- Do not import or use any visualization libraries.
...
- Absence of unauthorized imports (no visualization libs).
...
def generate_data_analysis_code(filedetails, business_problem, file_path, eda_guidance, processed_file_path=PROCESSED_DATASET_PATH):
    if 'error' in filedetails:
        return f"# Error: {filedetails['error']}"
    details = filedetails.copy()
    sample = details.get('sample_data', [])[:3]
...
def generate_and_refine_eda_code(filedetails, business_problem, file_path, eda_guidance, processed_file_path=PROCESSED_DATASET_PATH, max_refinements=3):
    requirements_summary = (
    ...
    code = generate_data_analysis_code(filedetails, business_problem, file_path, eda_guidance, processed_file_path)
    for i in range(max_refinements):
        ...
        if critique.strip() == "<OK>":
            return code
        if not critique.startswith("Issues found:"):
            return code
        revision_prompt = (
            f"Revise the script to address the following issues:\n{critique}\n"  
            f"Ensure adherence to EDA plan and formatting. Output only raw Python code."
        )
